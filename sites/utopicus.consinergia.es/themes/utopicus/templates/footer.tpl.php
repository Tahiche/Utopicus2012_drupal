<div id="footer">
		<h2>ESPACIOS UTOPIC_US</h2>
		<!-- footer-area -->
		<div class="footer-area">
			<!-- box -->
			<div class="box">
				<img width="210" height="83" alt="image description" src="images/img-banner.gif">
				<strong class="title">Madrid Espacio_US1</strong>
				<address style="height: 38px;">C/ Concepción Jerónima, 22<br>28012 Madrid.</address>
				<dl>
					<dt>Tel:</dt>
					<dd>+34 913 663 252</dd>
				</dl>
			</div>
			<!-- box -->
			<div class="box">
				<img width="211" height="83" alt="image description" src="images/img-banner.gif">
				<strong class="title">Madrid Espacio_US2</strong>
				<address style="height: 38px;">C/ Duque de Rivas, 5 28012 Madrid.</address>
				<dl>
					<dt>Tel:</dt>
					<dd>+34 913 663 252</dd>
				</dl>
			</div>
			<!-- box -->
			<div class="box">
				<img width="211" height="83" alt="image description" src="images/img-banner.gif">
				<strong class="title">Open_US</strong>
				<address style="height: 38px;">Abre un espacio Utopic_Us en tu ciudad.</address>
				<a class="more" href="#">Más información</a>
			</div>
			<!-- facebook-box -->
			<div class="facebook-box">
				<!-- facebook -->
				<div class="facebook"><img width="210" height="60" alt="facebook" src="images/img-facebook.gif"></div>
				<strong class="title">Suscríbete a nuestro boletín</strong>
				<form action="#">
					<fieldset>
						<div class="row">
							<div class="text-field"><input type="text" value="TU NOMBRE"></div>
						</div>
						<div class="row">
							<div class="text-field"><input type="text" value="TU EMAIL"></div>
						</div>
						<div class="row">
							<strong class="btn"><input type="submit" value="SUSCRIBIRME">SUSCRIBIRME</strong>
						</div>
					</fieldset>
				</form>
			</div>
		</div>
		<!-- sidemap-area -->
		<div class="sidemap-area">
			<!-- banner -->
			<a class="banner" href="#"><img width="125" height="125" alt="banner" src="images/img-banner.gif"></a>
			<!-- box -->
			<div class="box">
				<strong class="title"><a href="#">COWORKING</a></strong>
				<ul>
					<li><a href="#">Tour</a></li>
					<li><a href="#">Comunidad</a></li>
					<li><a href="#">Espacios</a></li>
					<li><a href="#">Áreas</a></li>
					<li><a href="#">Tarifas y planes</a></li>
				</ul>
			</div>
			<!-- box -->
			<div class="box">
				<strong class="title"><a href="#">FORMACION</a></strong>
				<ul>
					<li><a href="#">Cursos</a></li>
					<li><a href="#">Dosis</a></li>
					<li><a href="#">Metodología</a></li>
					<li><a href="#">Solicitar información</a></li>
				</ul>
			</div>
			<!-- box -->
			<div class="box">
				<strong class="title"><a href="#">ACTIVIDADES</a></strong>
				<ul>
					<li><a href="#">Eventos</a></li>
					<li><a href="#">utopic_Sundays</a></li>
					<li><a href="#">utopic_TV</a></li>
					<li><a href="#">Transformadora</a></li>
				</ul>
			</div>
			<!-- box -->
			<div class="box">
				<strong class="title"><a href="#">MOSTRADOR</a></strong>
				<ul>
					<li><a href="#">Audiovisual</a></li>
					<li><a href="#">Arquitectura</a></li>
					<li><a href="#">Consultoría</a></li>
					<li><a href="#">Diseño y comunicación</a></li>
					<li><a href="#">Web</a></li>
					<li><a href="#">Música</a></li>
					<li><a href="#">Moda</a></li>
				</ul>
			</div>
			<!-- box -->
			<div class="box">
				<strong class="title"><a href="#">ESPACIOS</a></strong>
				<ul>
					<li><a href="#">Madrid US_1</a></li>
					<li><a href="#">Madrid US_2</a></li>
					<li><a href="#">utopic_Bar</a></li>
					<li><a href="#">utopic_Gallery</a></li>
					<li><a href="#">Open_US</a></li>
					<li><a href="#">Alquiler de Espacios</a></li>
				</ul>
			</div>
			<!-- box -->
			<div class="box">
				<strong class="title"><a href="#">QUE SOMOS</a></strong>
				<ul>
					<li><a href="#">El proyecto</a></li>
					<li><a href="#">El equipo</a></li>
					<li><a href="#">Noticias</a></li>
					<li><a href="#">Patrocinadores</a></li>
					<li><a href="#">Sala de prensa</a></li>
					<li><a href="#">Contacto</a></li>
				</ul>
			</div>
		</div>
		<!-- footer-bar -->
		<div class="footer-bar">
			<!-- footer-nav -->
			<ul class="footer-nav">
				<li><a href="#">Condiciones de uso</a></li>
				<li><a href="#">Política de privacidad</a></li>
				<li><a href="#">Mapa del sitio</a></li>
			</ul>
			<div class="alignright">
				<!-- social-networks -->
				<ul class="social-networks">
					<li><a class="facebook" href="#">facebook<span class="mask">&nbsp;</span></a></li>
					<li><a class="twitter" href="#">twitter<span class="mask">&nbsp;</span></a></li>
					<li><a class="igoogle" href="#">iGoogle<span class="mask">&nbsp;</span></a></li>
					<li><a class="vimeo" href="#">vimeo<span class="mask">&nbsp;</span></a></li>
					<li><a class="flickr" href="#">flickr<span class="mask">&nbsp;</span></a></li>
					<li><a class="rss" href="#">rss<span class="mask">&nbsp;</span></a></li>
				</ul>
				<!-- banner -->
				<a class="banner" href="#"><img width="157" height="33" alt="banner" src="images/img-banner.gif"></a>
			</div>
		</div>
		<span class="copyright">&copy; 2012 Utopicus Innovación Cultural SL - Todos los derechos reservados</span>
	</div>