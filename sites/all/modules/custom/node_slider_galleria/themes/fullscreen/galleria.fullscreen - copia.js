    ...
    init: function(options) {
      ...
      this.bind(Galleria.RESCALE, function() {
		...
		this.$('info, iefix').toggle(this.hasInfo());
			/****** MIO *********/
		if(iniadada==0){
		this.$('image-nav-left, image-nav-right').css('opacity',0.01).animate({opacity: 1}, 300,
		function() {
			//alert("oe");
            $(this).delay(1000).animate({opacity: 0},500);
        });
		
		
		tab.click();
		tab.animate({opacity: 1},2000,function() {$(this).click()});
		iniadada=1;
		}
